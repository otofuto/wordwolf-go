...
import (
	...
	"strings"
	"time"
	"wordwolf/pkg/account"
	"wordwolf/pkg/blog"
	"wordwolf/pkg/category"
	"wordwolf/pkg/database"
	"wordwolf/pkg/member"
	"wordwolf/pkg/pagehtml"
	"wordwolf/pkg/question"
	"wordwolf/pkg/room"
	"wordwolf/pkg/util"
...
func IndexHandle(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		if r.URL.Path == "/" {
			...
			if rm.Url == "" {
				context.RoomnameOg = "この部屋はクローズしました"
			} else {
	...
	if r.Method == http.MethodGet {
		...
		if mode == "waiting" {
			...
			if rm.Url != "" {
				...
				if !me_exist {
					ApiResponse(w, 403, -1, "あなたはメンバーに含まれていません")
					return
				}
				bts, _ := json.Marshal(ResultList{
					ResultType: 0,
			...
			if util.Isset(r, []string{"count"}) {
				...
				if err != nil {
					ApiResponse(w, 400, -1, "相手が選択されていません")
					return
				}
